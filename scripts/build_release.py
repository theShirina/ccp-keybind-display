...
ROOT = Path(os.environ.get("CCPKD_ROOT", str(Path(__file__).resolve().parents[1]))).resolve()
...
def build() -> Path:
    ...
    output = DIST / ("CCPKeybindDisplay-" + version() + ".zip")
    temporary = DIST / (output.name + ".tmp-" + uuid.uuid4().hex)
    try:
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            for relative in RELEASE_FILES:
                path = ADDON / relative
                info = zipfile.ZipInfo("CCPKeybindDisplay/" + relative, FIXED_TIME)
        ...
        with zipfile.ZipFile(temporary, "r") as archive:
            ...
            expected = ["CCPKeybindDisplay/" + relative for relative in RELEASE_FILES]
